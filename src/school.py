...
class School:
    ...
    def _filter_by(self, data: pd.DataFrame, key_name):
        return data[
            data[key_name].astype(str)
            == str(self.regon if key_name.lower() == "regon" else self.rspo)
        ]
    ...
    def _get_spending_for_year(self, year):
        ...
        data = data[
            (
                (data["report_year"] == str(year))
                & (data["report_period"].astype(int) > 2)
            )
            | (
                (data["report_year"] == str(year + 1))
                & (data["report_period"].astype(int) <= 2)
            )
        ]
        sum = 0.0
